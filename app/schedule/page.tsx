...
import { sessionsFetcher } from '@/lib/swr';
...
export default function SchedulePage() {
  ...
  const { data: sessions = [], isLoading: loading } = useSWR<Session[]>(
            ...
            {sessions.map((s, i) => (
              ...
              >
                <p className="font-semibold capitalize">
                  {s.task_type?.replace('_', ' ') ?? 'Study session'}
                </p>
                ...
                {s.moodAtStart && (
                  <p className="text-sm text-slate-500 mt-1">Mood: {s.moodAtStart}</p>
                )}
