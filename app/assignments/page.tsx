...
import { assignmentsFetcher, coursesFetcher, createAssignmentMutationFetcher } from '@/lib/swr';
...
export default function AssignmentsPage() {
  ...
  } = useSWR<Assignment[]>(user ? '/api/onboarding/assignments' : null, assignmentsFetcher);

  const { data: courses = [] } = useSWR<Course[]>(
    user ? '/api/onboarding/courses' : null,
    coursesFetcher,
  );

  const { trigger: createAssignment, isMutating: submitting } = useSWRMutation(
    'create-assignment',
    createAssignmentMutationFetcher,
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err?.data?.error || 'Failed to create assignment');
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, estimated_minutes: Number(e.target.value) || 30 }))
                  }
                  className="w-full px-4 py-2 rounded-lg bg-slate-800 border border-slate-700 focus:border-primary outline-none"
                />
              </div>
