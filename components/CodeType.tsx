...
import { Code2, Zap, Target, Trophy, RotateCcw, ChevronDown, Sparkles } from 'lucide-react';
import { saveCodingSessionMutationFetcher, generatePracticeCodeMutationFetcher } from '@/lib/swr';
...
const SUPPORTED_AI_LANGUAGES: Language[] = [
  'javascript',
  'python',
  'typescript',
  'java',
...
interface CodeTypeProps {
  mood?: 'low' | 'moderate' | 'high' | null;
  timeAvailable?: number;
}

export default function CodeType({ mood = null, timeAvailable = 30 }: CodeTypeProps) {
  ...
  const { trigger: saveCodingSession } = useSWRMutation(
    'save-coding-session',
    saveCodingSessionMutationFetcher,
  );

  const { trigger: generatePracticeCode, isMutating: generatingCode } = useSWRMutation(
    'generate-practice-code',
    generatePracticeCodeMutationFetcher,
  );

  const handleGenerateWithAI = async () => {
    const effectiveMood = mood ?? 'moderate';
    const effectiveTime = timeAvailable ?? 30;
    try {
      const data = await generatePracticeCode({
        mood: effectiveMood,
        timeAvailable: effectiveTime,
        language: SUPPORTED_AI_LANGUAGES.includes(language) ? language : undefined,
      });
      if (data?.code) {
        ...
        if (data.language && SUPPORTED_AI_LANGUAGES.includes(data.language as Language)) {
          setLanguage(data.language as Language);
        }
      }
    } catch (e) {
      console.error('Failed to generate practice code:', e);
    }
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    // Check if complete — save session and set completion in one place
    if (value === codeSnippet) {
      const endedAt = Date.now();
      setEndTime(endedAt);
      setIsComplete(true);
      const start = startTime ?? endedAt;
      if (!savedForCompletionRef.current) {
        savedForCompletionRef.current = true;
        const durationSeconds = Math.round((endedAt - start) / 1000);
        ...
        const accuracyPercent =
          codeSnippet.length > 0 ? Math.round((correctChars / codeSnippet.length) * 100) : 100;
        saveCodingSession({
          language,
          ...
          wpm,
          accuracyPercent,
        }).catch((err) => console.error('Failed to save typing session:', err));
      }
    }
  };
  ...
  const getCharacterClass = (index: number) => {
    if (index < currentIndex) {
      return userInput[index] === codeSnippet[index]
        ? 'text-green-400'
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
                ...
                >
                  {Object.keys(CODE_SNIPPETS).map((lang) => (
                    <button
                      ...
                      className={`w-full px-4 py-2 text-left hover:bg-primary/20 transition-colors capitalize ${
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary/20 border border-primary/50 hover:bg-primary/30 text-primary transition-colors disabled:opacity-50"
            title="Generate practice code with AI based on your mood and time"
          >
            <Sparkles className="w-4 h-4" />
            {generatingCode ? 'Generating…' : 'Generate with AI'}
          </button>
          <button
            onClick={resetGame}
            className="p-2 rounded-lg bg-slate-800 border border-slate-700 hover:border-primary transition-colors"
            title="Reset with random snippet"
          >
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
          ...
          <div className="text-2xl font-bold">
            {isComplete ? calculateWPM() : (getCurrentWPM() ?? '--')}
          </div>
        </div>
        <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
          <div className="flex items-center gap-2 mb-1">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-300">
                  You typed at <span className="font-bold text-primary">{calculateWPM()} WPM</span>{' '}
                  with{' '}
